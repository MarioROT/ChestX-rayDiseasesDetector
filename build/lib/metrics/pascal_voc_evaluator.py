...
def get_pascalvoc_metrics(
    ...
    for c, v in classes_bbs.items():
        ...
        ap, mpre, mrec = None, None, None
        if method == MethodAveragePrecision.EVERY_POINT_INTERPOLATION:
            print('metrics defined in a normal way')
        ...
        if not ap and not mpre and not mrec:
            print('metrics not defined in a normal way; line 218 metrics.pascal_voc_evaluator')
            [ap, mpre, mrec, ii] = calculate_ap_every_point(rec, prec)
